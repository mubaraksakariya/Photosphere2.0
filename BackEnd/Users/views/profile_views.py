...
class ProfileViewSet(ModelViewSet):
    ...
    def update(self, request, *args, **kwargs):
        ...
        if remove_image and profile.profile_image:
            default_image = profile._meta.get_field(
                'profile_image').get_default()
            if profile.profile_image != default_image:
                profile.profile_image.delete()  # Deletes the uploaded image from storage
                profile.profile_image = default_image  # Set default image
